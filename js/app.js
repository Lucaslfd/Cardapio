...
var CELULAR_EMPRESA = '5531988499320'
...
cardapio.metodos = {
    ...
    obterItensCardapio: (categoria = 'burgers', vermais = false) => {
        ...
        //seta para o menu ativo
        $("#menu-" + categoria).addClass('active')
    },

    //click no botao ver mais
    verMais: () => {
    ...
    adicionarAoCarrinho: (id) => {
        ...
        if (qntdAtual > 0) {
            ...
            if (item.length > 0) {
                ...
                //caso ainda não exita o item no carrinho adc ele
                else {
    ...
    // autra os texto e exibe os botoes das etapas
    carregarEtapa: (etapa) => {
    ...
    carregarCarrinho: () => {
        ...
        if (MEU_CARRINHO.length > 0) {
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                ...
                let temp = cardapio.templates.itemCarrinho.replace(/\${img}/g, e.img).replace(/\${name}/g, e.name).replace(/\${price}/g, e.price.toFixed(2).replace('.', ',')).replace(/\${id}/g, e.id).replace(/\${qntd}/g, e.qntd)

                $("#itensCarrinho").append(temp)

                //atualiza os valores R$ totais do carrinhoq quando for o ultimo item
    ...
    // aumentar quantidae do item do carrinho
    ...
    carregarValores: () => {
        ...
        $("#lblValorEntrega").text('+ RS 0,00')
        $("#lblValorTotal").text('RS 0,00')
    ...
    //validaçao antes de prosseguir para a etapa 3
    resumoPedido: () => {
    ...
    carregarResumo: () => {
        ...
        $.each(MEU_CARRINHO, (i, e) => {
            ...
            let temp = cardapio.templates.itemResumo.replace(/\${img}/g, e.img).replace(/\${name}/g, e.name).replace(/\${price}/g, e.price.toFixed(2).replace('.', ',')).replace(/\${qntd}/g, e.qntd)
    ...
    mensagem: (texto, cor = 'red', tempo = 3500) => {
        ...
        let msg = `<div id="msg-${id}" class="animate__fadeInDown toast ${cor}">${texto}</div>`;
